...
export default function Estimate() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`https://immaculate-qlaf.vercel.app/api/estimatedata/${userid}`, {
        headers: {
  ...
  const handleConvertToInvoice = async (estimateid) => {
    try {
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`https://immaculate-qlaf.vercel.app/api/converttoinvoice/${estimateid}`, {
        method: 'POST',
        headers: {
          'Authorization': authToken,
        }
