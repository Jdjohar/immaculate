...
export default function Dashboard() {
  ...
  const handleClockIn = async () => {
    ...
    try {
      ...
      const response = await fetch('hhttps://immaculate.onrender.com/api/clockin', {
        method: 'POST',
        headers: {
  ...
  const handleClockOut = async () => {
    ...
    try {
      ...
      let isTeamMember = localStorage.getItem('isTeamMember');
      const authToken = localStorage.getItem('authToken');
      const response = await fetch('hhttps://immaculate.onrender.com/api/clockout', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': authToken,
      ...
      if (response.status === 401) {
        ...
        console.log(new Date(startTime).getTime(), "Start");
  ...
  const fetchUserEntries = async (start, end) => {
    try {
      const userid = localStorage.getItem('userid');
      const authToken = localStorage.getItem('authToken');
      const response = await fetch(`hhttps://immaculate.onrender.com/api/userEntries/${userid}`, {
        headers: {
          'Authorization': authToken,
        }
      });
